    ...
    private int bufferCopiedCallback(long nCopiedSize, long nAvailableSize) {
		...
		if(mVideoFrame == null || mWidth != frameWidth || frameHeight != mHeight || mVideoFrame.capacity() != nAvailableSize){
			synchronized(mPreview){
				MyLog.d(TAG, "VideoConsumer  frameW/H:"+(frameWidth+"/"+frameHeight)+"  dataW/H:"+(dataWidth+"/"+dataHeight));
...
//	    	MyLog.d(TAG, ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> setViewport()");
	    	...
	    	WindowManager wm = (WindowManager) NgnApplication.getContext().getSystemService(Context.WINDOW_SERVICE);
	    	wm.getDefaultDisplay().getMetrics(dm);
	    	
	    	float fRatio = ((float) width / (float) height);
	    	mViewWidth = (int) ((float) dm.widthPixels / fRatio) > dm.heightPixels ? (int) ((float) dm.heightPixels * fRatio) : dm.widthPixels;
			mViewHeight = (int) (mViewWidth / fRatio) > dm.heightPixels ? dm.heightPixels : (int) (mViewWidth / fRatio);
			mViewX = ((dm.widthPixels - mViewWidth) >> 1);
			mViewY = ((dm.heightPixels - mViewHeight) >> 1);
			MyLog.d(TAG, "VideoConsumer Preview W/H:"+(mViewWidth+"/"+mViewHeight));
