...
public class NgnProxyAudioConsumer extends NgnProxyPlugin {
	...
	private Object mLocked = new Object();// 锟斤�
	...
	//�յ�PTT��ռ ����������ݻ���	
		if(!pause){
			Log.d("HX-0326", "------PTT------mConsumer.reset();------");
			setOnResetJB();
		}
		mIsPaused = pause;
		if (mPaused == pause) {
...
//		}
		...
		} else if (NgnApplication.isBh()) { // 锟叫匡拷院锟斤拷锟斤拷PAD锟斤拷锟街筹拷台
			if (audiomanager.getMode() != AudioManager.MODE_IN_COMMUNICATION) {
				audiomanager.setMode(AudioManager.MODE_IN_COMMUNICATION);
			}

			audiomanager.setSpeakerphoneOn(speakerOn);
		}else if (NgnApplication.isl8848a_l1860()&& !GlobalSession.bSocketService) {  //lc1810 l8848(锟斤拷芯锟斤拷锟秸讹�) l1860(锟斤拷芯PAD)  
			...
			while (NgnProxyAudioConsumer.super.mValid
				...
				for (i = 0; i < nFramesCount; i++) {
					...
					if (lSizeInBytes > 0) {
						...
						reBlankCounts++;
						//lSize0count ++;
						setOnResetJB();// gl解决断网重联后语音单通问�
						//if(lSize0count == 20){
						//	setOnResetJB();// gl锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟酵拷锟斤拷锟�
						//	lSize0count = 0;
...
//								Log.d(TAG,"NgnAVSession 1 reBlankCounts = "+reBlankCounts);
								if(GlobalVar.bADHocMode || (ngnAVSession.getSessionType() == SessionType.AudioCall || ngnAVSession.getSessionType() == SessionType.VideoCall))
									MyLog.d(TAG, "bADHocMode = " + GlobalVar.bADHocMode + ", SessionType = " + ngnAVSession.getSessionType());
					...
					//MyLog.d(TAG, "mIsPaused: " +mIsPaused);
				}

//				Log.e("", "锟秸碉拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤� "+new String(aAudioBytes));
				 ...
				 */
				if(!mIsPaused){
					mAudioTrack.write(aAudioBytes, 0, aAudioBytes.length); // 锟斤拷锟斤拷PAD锟斤拷锟斤拷没锟斤拷锟斤拷锟斤拷
				}
				// }
