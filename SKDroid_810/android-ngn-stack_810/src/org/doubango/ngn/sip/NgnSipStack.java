...
public class NgnSipStack extends SipStack {
	 ...
	 * @param impuUri
	 */
	public NgnSipStack(SipCallback callback, String realmUri, String impiUri,
			String impuUri) {
		...
		// Set first and second DNS servers (used for DNS NAPTR+SRV discovery
		// and ENUM)
		String dnsServer = null;
		// 只获取dns1地址
		if ((dnsServer = mNetworkService.getDnsServer(DNS_TYPE.DNS_1)) != null
				&& !dnsServer.equals("0.0.0.0")) {
			this.addDnsServer(dnsServer);
			// if((dnsServer = mNetworkService.getDnsServer(DNS_TYPE.DNS_2)) !=
			// null && !dnsServer.equals("0.0.0.0")){
			// this.addDnsServer(dnsServer);
			// MyLog.d(TAG, "sipstack add dns:"+dnsServer);
			// }
		} else {
			// On the emulator FIXME
			NgnSipPrefrences ngnSipPrefrences = new NgnSipPrefrences();// 如果dns地址获取失败，那就设置核心网的地址
			String pcscfHost = ngnSipPrefrences.getPcscfHost();
			if (!TextUtils.isEmpty(pcscfHost)) {
				this.addDnsServer(pcscfHost);
			} else {
