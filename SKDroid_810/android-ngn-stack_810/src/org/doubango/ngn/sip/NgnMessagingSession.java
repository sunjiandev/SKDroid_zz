...
public class NgnMessagingSession extends NgnSipSession {
    ...
    {
    	super.addHeader("APP_Type", NgnContentType.GPS_APP_TYPE);//IMS messaging/DDS-mode messaging
    	super.addHeader("Content-Type", NgnContentType.GPS_CONTENT_TYPE_HEADER_COMMAND);//application/dds-gps+xml
    ...
    public boolean sendGPSMessage_data(String text) {
    	super.addHeader("APP_Type", NgnContentType.GPS_APP_TYPE); //IMS messaging/DDS-mode messaging
    	super.addHeader("Content-Type", NgnContentType.GPS_CONTENT_TYPE_HEADER_DATA); //application/dds-gps-data+xml
...
//	super.addHeader("Route", mConfigurationService.getString(NgnConfigurationEntry.SERVICE_ROUTE, NgnConfigurationEntry.DEFAULT_SERVICE_ROUTE));
   ...
   public boolean sendExWithListTextMessage(String text, String contentType){
   	if (!NgnStringUtils.isNullOrEmpty(contentType)) {
   		...
   		super.addHeader("Content-Type", "multipart/mixed;boundary=\"boundary1\"");
   		super.addHeader("Content-Disposition","render");
   		super.addHeader("Require", "recipient-list-message");
   		super.addHeader("APP_Type", " IMS messaging/Page-mode messaging/CHINA MOBILE");
   	}
   	byte[] bytes = text.getBytes();
       ByteBuffer payload = ByteBuffer.allocateDirect(bytes.length);
       payload.put(bytes);
