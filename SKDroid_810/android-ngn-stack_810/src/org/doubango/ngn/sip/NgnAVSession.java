...
public class NgnAVSession extends NgnInviteSession{
    ...
    public static NgnAVSession takeIncomingSession(NgnSipStack sipStack, CallSession session, twrap_media_type_t mediaType, SipMessage sipMessage){
        ...
        synchronized (sSessions){
            ...
            final NgnAVSession avSession = new NgnAVSession(sipStack, session, media, InviteState.INCOMING);
            if (sipMessage != null){
    ...
    public static boolean handleMediaUpdate(long id, twrap_media_type_t newMediaType){
        ...
        if (avSession != null){
        	...
        	if(_newMediaType != NgnMediaType.None){
        		avSession.mOldMediaType = avSession.mMediaType;//gzc 20141023记住session原来的媒体类型 
...
//            	}
     ...
     */
    public static boolean makeAudioCall(String remoteUri, NgnSipStack sipStack, int flag){
     ...
     */
    public static boolean makeAudioVideoCall(String remoteUri, NgnSipStack sipStack, int flag){
...
//           		super.addHeader("Route", mConfigurationService.getString(NgnConfigurationEntry.SERVICE_ROUTE, NgnConfigurationEntry.DEFAULT_SERVICE_ROUTE));
...
//						mVideoProducer.setSipSessionId(super.getId());
						...
						if (NgnApplication.getSDKVersion() >= 14) {
							...
							mVideoProducer_surface.setSipSessionId(super.getId());
						}
					}
				}
			}
...
//					else { //for test
//						MyMyLog.d(TAG, "mAudioConsumer - myProxyPlugin = " + myProxyPlugin);
//					}
				}
//				else { //for test
//					MyMyLog.d(TAG, "findProxyPluginConsumer - plugin = " + plugin);
//				}
...
//					else { //for test
//						MyMyLog.d(TAG, "mAudioProducer - myProxyPlugin = " + myProxyPlugin);
//					}
				}
//				else { //for test
//					MyMyLog.d(TAG, "findProxyPluginProducer - plugin = " + plugin);
//				}
			}
//			else { //for test
	...
	private void updateEchoTail(){
		if(mConfigurationService.getBoolean(NgnConfigurationEntry.GENERAL_USE_ECHO_TAIL_ADAPTIVE,
			...
			if((mediaMgr = super.getMediaSessionMgr()) != null){
				...
				MyLog.d(TAG, "getMinBufferSize("+samplingRate+ ","+channels+")="+minBufferSize);
...
//		}
	...
	public void setmSendVIdeo(boolean mSendVIdeo){
		...
		MyLog.d(TAG, "setmSendVIdeo: " + GlobalVar.mSendVideo);
	}
...
//		}
		...
		if (NgnApplication.getSDKVersion() >= 14) {
			...
			if(mVideoProducer_surface != null){
				return mVideoProducer_surface.getNativeCameraHardRotation(preview);
			}
		}
		return 0;
	}
...
//				initializeConsumersAndProducers();
	...
	/*
	 * transferToDisplayName can be NULL
	 * transferToPhoneNum cann't be NULL
	 * 
	 * eg: sendTransfer(null, "19800005005")
	 * */
	public boolean sendTransfer(String transferToDisplayName,String transferToPhoneNum){
        if (NgnStringUtils.isNullOrEmpty(transferToPhoneNum) ){
            return false;
        }
        
        return super.isActive() ? mSession.sendTransfer(transferToDisplayName, transferToPhoneNum) : false;
	...
	public boolean transferCall(String transferUri, NgnAVSession destSession){
        ...
        if(destSession == null){
            ...
            return super.isActive() ? mSession.transfer(transferUri,(CallSession) destSession.getSession()) : false;
        }
...
//sks add start
	...
	public boolean isGroupCall() {
		return isGroupAudioCall() || isGroupVideoCall()|| isGroupVideoMoniter();
	}
	...
	public boolean isGroupVideoMoniter() {
		if(sessionType == SessionType.GroupVideoMonitor)
			return true;
		else 
			return false;
	}
    ...
    {
    	if(apptype.contains("groupcall/audio"))
    	{
    		sessionType = SessionType.GroupAudioCall;
    	}
    	else if(apptype.contains("groupcall/video"))
    	{
    		
    		sessionType = SessionType.GroupVideoCall;
    		if(apptype.contains("monitor"))//video/
	    	{
	    		sessionType = SessionType.GroupVideoMonitor;
	    	}
    	}
    	else if(apptype.equalsIgnoreCase("encryptcall"))
    	{
    		sessionType = SessionType.EncryptCall;
    	}
    	else if(apptype.equalsIgnoreCase("monitor"))//video/
    	{
    		sessionType = SessionType.VideoMonitor;
    	}
    	else if(apptype.equalsIgnoreCase("surveil")|| apptype.equalsIgnoreCase("monitor/surveil"))//video/调度台发起的视频回传passive-monitor
    	{
    		sessionType = SessionType.VideoSurveilMonitor;
    	}
    	else if(apptype.equalsIgnoreCase("groupcall/video/monitor"))//video/
    	{
    		sessionType = SessionType.GroupVideoMonitor;
    	}
    ...
    public void setSessionType(String apptype)
    {
    	...
    	/*
    	 * 监控类：(four)
    	 * whether to answer the phone automatically or not
    	 * 视频监控、 调度台发起的主动视频回传、终端发起的视频回传、组监控
    	 */
    ...
    public void checkNetwork(){
    	 if(this.isGroupAudioCall() || this.isGroupVideoCall() || this.isVideoMonitorCall()){
         	MyLog.d(TAG, "This session: " + this.sessionType + " not check network.");
         	MediaSessionMgr.defaultsSetCheckNetworkEnabled(false);
         }
         else{
         	MyLog.d(TAG, "This session: " + this.sessionType + " check network.");
