...
public class ScreenQoS extends BaseScreen {
	...
	private final static String[] sSpinnerRefresherItems = new String[] {
			NgnConfigurationEntry.DEFAULT_QOS_REFRESHER, "uas", "uac" };
	private final static ScreenQoSStrength[] sSpinnerPrecondStrengthItems = new ScreenQoSStrength[] {
			new ScreenQoSStrength(
					tmedia_qos_strength_t.tmedia_qos_strength_none, "None"),
			new ScreenQoSStrength(
					tmedia_qos_strength_t.tmedia_qos_strength_optional,
					"Optional"),
			new ScreenQoSStrength(
					tmedia_qos_strength_t.tmedia_qos_strength_mandatory,
					"Mandatory"), };
	private final static ScreenQoSType[] sSpinnerPrecondTypeItems = new ScreenQoSType[] {
			...
			new ScreenQoSType(tmedia_qos_stype_t.tmedia_qos_stype_segmented,
					"Segmented"),
			new ScreenQoSType(tmedia_qos_stype_t.tmedia_qos_stype_e2e,
					"End2End") };
...
//		new ScreenQoSVsize("QSKD (256 x 144)",
//				tmedia_pref_video_size_t.tmedia_pref_video_size_qskd),
		new ScreenQoSVsize("SKD (512 x 288)",
				tmedia_pref_video_size_t.tmedia_pref_video_size_skd),
//		new ScreenQoSVsize("LSKD (768 x 432)",
//				tmedia_pref_video_size_t.tmedia_pref_video_size_lskd),
...
//					tmedia_pref_video_size_t.tmedia_pref_video_size_qvga),
			new ScreenQoSVsize("CIF (352 x 288)",
...
//					tmedia_pref_video_size_t.tmedia_pref_video_size_hvga),
			new ScreenQoSVsize("VGA (640 x 480)",
...
//					tmedia_pref_video_size_t.tmedia_pref_video_size_1080p)
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		ArrayAdapter<String> adapter = new ArrayAdapter<String>(this,
				...
				ScreenQoS.sSpinnerRefresherItems);
		adapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
		mSpRefresher.setAdapter(adapter);

		ArrayAdapter<ScreenQoSStrength> adapterStrength = new ArrayAdapter<ScreenQoSStrength>(
				this, android.R.layout.simple_spinner_item,
				ScreenQoS.sSpinnerPrecondStrengthItems);
		adapterStrength
				.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
		mSpPrecondStrength.setAdapter(adapterStrength);

		ArrayAdapter<ScreenQoSType> adapterType = new ArrayAdapter<ScreenQoSType>(
				this, android.R.layout.simple_spinner_item,
				ScreenQoS.sSpinnerPrecondTypeItems);
		adapterType
				.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
		...
		mCbEnableSessionTimers.setChecked(mConfigurationService.getBoolean(
		...
		mEtSessionTimeOut
				.setText(mConfigurationService.getString(
						...
						Integer.toString(NgnConfigurationEntry.DEFAULT_QOS_SIP_CALLS_TIMEOUT)));
		mSpRefresher.setSelection(getSpinnerIndex(mConfigurationService
				.getString(NgnConfigurationEntry.QOS_REFRESHER,
						sSpinnerRefresherItems[0]), sSpinnerRefresherItems));

		mSpPrecondStrength
				.setSelection(ScreenQoSStrength.getSpinnerIndex(tmedia_qos_strength_t.valueOf(mConfigurationService
						.getString(
								NgnConfigurationEntry.QOS_PRECOND_STRENGTH,
								NgnConfigurationEntry.DEFAULT_QOS_PRECOND_STRENGTH))));

		mSpPrecondType
				.setSelection(ScreenQoSType.getSpinnerIndex(tmedia_qos_stype_t.valueOf(mConfigurationService
		...
		mRlSessionTimers
				.setVisibility(mCbEnableSessionTimers.isChecked() ? View.VISIBLE
						: View.INVISIBLE);
		...
		addConfigurationListener(mEtNetworkQos);
		// add local listeners
		mCbEnableSessionTimers
				.setOnCheckedChangeListener(mCbEnableSessionTimers_OnCheckedChangeListener);
	}
	...
	protected void onPause() {
		if (super.mComputeConfiguration) {
			...
			mConfigurationService.putString(
					...
					sSpinnerRefresherItems[mSpRefresher
							.getSelectedItemPosition()]);
			mConfigurationService.putString(
					NgnConfigurationEntry.QOS_PRECOND_STRENGTH,
					sSpinnerPrecondStrengthItems[mSpPrecondStrength
							.getSelectedItemPosition()].mStrength.toString());
			mConfigurationService.putString(
					NgnConfigurationEntry.QOS_PRECOND_TYPE,
					sSpinnerPrecondTypeItems[mSpPrecondType
							.getSelectedItemPosition()].mType.toString());
			mConfigurationService
					.putString(NgnConfigurationEntry.QOS_PREF_VIDEO_SIZE,
							sSpinnerVsizeItems[mSpVsize
									.getSelectedItemPosition()].mValue //9 720P (1280 x 720) tmedia_pref_video_size_720p
									.toString());
...
//				MediaSessionMgr
//						.defaultsSetPrefVideoSize(sSpinnerVsizeItems[mSpVsize
//								.getSelectedItemPosition()].mValue);    // del by gle
				...
				int losePackets = mConfigurationService.getInt(NgnConfigurationEntry.NETWORK_QOS_LOSEPACKETS, 
						NgnConfigurationEntry.DEFAULT_NETWORK_QOS_LOSEPACKETS);
				Log.d(TAG,String.format("set FPS=%d, losePackets=%d",fps,losePackets));
	...
	private OnCheckedChangeListener mCbEnableSessionTimers_OnCheckedChangeListener = new OnCheckedChangeListener() {
		public void onCheckedChanged(CompoundButton arg0, boolean isChecked) {
			mRlSessionTimers.setVisibility(isChecked ? View.VISIBLE
					: View.INVISIBLE);
	...
	private static class ScreenQoSStrength {
		...
		private ScreenQoSStrength(tmedia_qos_strength_t strength,
				String description) {
			mStrength = strength;
