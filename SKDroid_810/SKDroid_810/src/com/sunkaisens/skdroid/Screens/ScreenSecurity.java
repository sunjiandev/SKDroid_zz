...
public class ScreenSecurity extends BaseScreen {
	...
	private final static ScreenSecuritySRtpMode sSpinnerSRtpModeItems[] = new ScreenSecuritySRtpMode[] {
			new ScreenSecuritySRtpMode(
					tmedia_srtp_mode_t.tmedia_srtp_mode_none, "None"),
			new ScreenSecuritySRtpMode(
					tmedia_srtp_mode_t.tmedia_srtp_mode_optional, "Optional"),
			new ScreenSecuritySRtpMode(
					tmedia_srtp_mode_t.tmedia_srtp_mode_mandatory, "Mandatory"), };
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		// load values from configuration file (do it before adding UI
		// listeners)
		mEtAMF.setText(mConfigurationService.getString(
				NgnConfigurationEntry.SECURITY_IMSAKA_AMF,
				NgnConfigurationEntry.DEFAULT_SECURITY_IMSAKA_AMF));
		mEtOpId.setText(mConfigurationService.getString(
				NgnConfigurationEntry.SECURITY_IMSAKA_OPID,
				NgnConfigurationEntry.DEFAULT_SECURITY_IMSAKA_OPID));
		// mEtPrivKey.setText(mConfigurationService.getString(CONFIGURATION_SECTION.SECURITY,
		// CONFIGURATION_ENTRY.TLS_PRIV_KEY_FILE,
		// Configuration.DEFAULT_TLS_PRIV_KEY_FILE));
		// mEtPubKey.setText(mConfigurationService.getString(CONFIGURATION_SECTION.SECURITY,
		// CONFIGURATION_ENTRY.TLS_PUB_KEY_FILE,
		// Configuration.DEFAULT_TLS_PUB_KEY_FILE));
		// mEtCA.setText(mConfigurationService.getString(CONFIGURATION_SECTION.SECURITY,
		// CONFIGURATION_ENTRY.TLS_CA_FILE, Configuration.DEFAULT_TLS_CA_FILE));
		// mCbTlsSecAgree.setChecked(mConfigurationService.getBoolean(CONFIGURATION_SECTION.SECURITY,
		// CONFIGURATION_ENTRY.TLS_SEC_AGREE,
		// Configuration.DEFAULT_TLS_SEC_AGREE));

		ArrayAdapter<ScreenSecuritySRtpMode> adapter = new ArrayAdapter<ScreenSecuritySRtpMode>(
				this, android.R.layout.simple_spinner_item,
				ScreenSecurity.sSpinnerSRtpModeItems);
		adapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
		mSpSRtpMode.setAdapter(adapter);

		mSpSRtpMode
				.setSelection(ScreenSecuritySRtpMode.getSpinnerIndex(tmedia_srtp_mode_t.valueOf(mConfigurationService
						.getString(
		...
		mCbTlsSecAgree
				.setOnCheckedChangeListener(cbTlsSecAgree_OnCheckedChangeListener);
		mCbTlsFiles
				.setOnCheckedChangeListener(cbTlsFiles_OnCheckedChangeListener);
	}
	...
	protected void onPause() {
		if (super.mComputeConfiguration) {
			...
			mConfigurationService.putString(
					NgnConfigurationEntry.SECURITY_IMSAKA_AMF, mEtAMF.getText()
							.toString());
			mConfigurationService.putString(
					NgnConfigurationEntry.SECURITY_IMSAKA_OPID, mEtOpId
							.getText().toString());
			mConfigurationService
					.putString(NgnConfigurationEntry.SECURITY_SRTP_MODE,
							sSpinnerSRtpModeItems[mSpSRtpMode
									.getSelectedItemPosition()].mMode
									.toString());

			// configurationService.setString(CONFIGURATION_SECTION.SECURITY,
			// CONFIGURATION_ENTRY.TLS_PRIV_KEY_FILE,
			// etPrivKey.getText().toString());
			// configurationService.setString(CONFIGURATION_SECTION.SECURITY,
			// CONFIGURATION_ENTRY.TLS_PUB_KEY_FILE,
			// etPubKey.getText().toString());
			// configurationService.setString(CONFIGURATION_SECTION.SECURITY,
			// CONFIGURATION_ENTRY.TLS_CA_FILE, etCA.getText().toString());

			if (!mConfigurationService.commit()) {
				Log.e(TAG, "Failed to Compute() configuration");
			} else {
				MediaSessionMgr
						.defaultsSetSRtpMode(sSpinnerSRtpModeItems[mSpSRtpMode
								.getSelectedItemPosition()].mMode);
			}
	...
	public void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (requestCode == ScreenSecurity.REQUEST_CODE_PRIV_KEY) {
			...
			Log.e(ScreenSecurity.TAG,
					String.format("%d is an unknown request code", requestCode));
		}
	}
	...
	private OnClickListener ibPrivKey_OnClickListener = new OnClickListener() {
		public void onClick(View v) {
			...
			// ServiceManager.getMainActivity().startActivityForResult(intent,
			// 1);

			/*
			 * Intent intent = new Intent(); Uri startDir = Uri.fromFile(new
			 * File("/sdcard")); intent.setAction(Intent.ACTION_PICK);
			 * intent.setDataAndType(startDir, "vnd.android.cursor.dir/*");
			 * //intent.setDataAndType(startDir, "file://"); // Title
			 * intent.putExtra("explorer_title", "Select a file"); // Optional
			 * colors intent.putExtra("browser_title_background_color",
			 * "440000AA"); intent.putExtra("browser_title_foreground_color",
			 * "FFFFFFFF"); intent.putExtra("browser_list_background_color",
			 * "00000066"); // Optional font scale
			 * intent.putExtra("browser_list_fontscale", "120%"); // Optional
			 * 0=simple list, 1 = list with filename and size, 2 = list with
			 * filename, size and date. intent.putExtra("browser_list_layout",
			 * "2"); startActivityForResult(intent, 999);
			 */

			/*
			 * Intent intent = new Intent(); Uri directory = Uri.fromFile(new
			 * File("/sdcard")); intent.setAction(Intent.ACTION_PICK);
			...
			// ScreenSecurity.startActivityForResult(new
			// Intent(Intent.ACTION_GET_CONTENT),
			// ScreenSecurity.REQUEST_CODE_PRIV_KEY);
		}
	};
	...
	private OnCheckedChangeListener cbTlsFiles_OnCheckedChangeListener = new OnCheckedChangeListener() {
		public void onCheckedChanged(CompoundButton arg0, boolean isChecked) {
			...
			Toast.makeText(ScreenSecurity.this, "Not implemented",
	...
	private OnCheckedChangeListener cbTlsSecAgree_OnCheckedChangeListener = new OnCheckedChangeListener() {
		public void onCheckedChanged(CompoundButton arg0, boolean isChecked) {
			// mConfigurationService.setBoolean(CONFIGURATION_SECTION.SECURITY,
			// CONFIGURATION_ENTRY.TLS_SEC_AGREE, isChecked);
		}
	};
	...
	private static class ScreenSecuritySRtpMode {
		...
		private ScreenSecuritySRtpMode(tmedia_srtp_mode_t mode,
				String description) {
			mMode = mode;
