...
public class ScreenFileTransferView extends BaseScreen {
	private static final String TAG = ScreenFileTransferView.class
			.getCanonicalName();
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mSession = NgnMsrpSession.getSession(NgnStringUtils.parseLong(
				super.mId, -1));
		if (mSession == null) {
			Log.e(TAG, String.format("Cannot find MSRP session with id=%s",
					super.mId));
			finish();
			mScreenService.show(ScreenTabHome.class);
			return;
		...
		String remoteParty = mSession.getToUri();
		ModelContact mc = SystemVarTools.createContactFromRemoteParty(mSession
				.getRemotePartyUri());
		mTvRemoteParty.setText(mc.name + "(" + mc.mobileNo + ")");
		// mTvInfo.setText(mSession.isOutgoing()? "Sending Content...":
		// "Receiving Content...");
		mTvInfo.setText(mSession.isOutgoing() ? ScreenFileTransferView.this
				.getString(R.string.sending_file) : ScreenFileTransferView.this
				.getString(R.string.receiving_file));
		mBtAccept
				.setVisibility(mSession.isOutgoing() || mSession.isConnected() ? View.GONE
						: View.VISIBLE);
		// mBtAbort.setText(mSession.isConnected()? "Abort" :
		// (mSession.isOutgoing() ? "Cancel" : "Decline"));
		mBtAbort.setText(mSession.isConnected() ? ScreenFileTransferView.this
				.getString(R.string.break_off)
				: (mSession.isOutgoing() ? ScreenFileTransferView.this
						.getString(R.string.file_cacel)
						: ScreenFileTransferView.this
		...
		if (mSession.isOutgoing()) {
			...
			if (filePath != null) {
				try {
					mIvPreview.setImageURI(new Uri.Builder().path(filePath)
							.build());
				} catch (Exception e) {
		...
		updateProgressBar(mSession.getStart(), mSession.getEnd(),
				mSession.getTotal());
		...
		mBtAccept.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View v) {
				boolean result = mSession.accept();
				if (result != true) {
					Log.d(TAG, "session.accept failed!! result = false.");
				}
			}
		});
	}
	...
	private final TimerTask mTimerTaskSuicide = new TimerTask() {
		...
		public void run() {
			runOnUiThread(new Runnable() {
				public void run() {
					final IBaseScreen currentScreen = mScreenService
							.getCurrentScreen();
					if (currentScreen != null
							&& currentScreen.getType() == SCREEN_TYPE.FILETRANSFER_VIEW_T) {
						mScreenService.show(ScreenTabHome.class);
						mScreenService.destroy(getId());
					}
				}
	...
	private void handleSipAndMsrpEvent(Intent intent) {
		...
		if (NgnInviteEventArgs.ACTION_INVITE_EVENT.equals(action)) {
			final NgnInviteEventArgs args = intent
			...
			case INCALL:
				mRelativeLayout
						.setBackgroundResource(R.drawable.grad_bkg_incall);
		...
		// MSRP events
			...
			final NgnMsrpEventArgs args = intent
			...
			case CONNECTED:
				...
				mBtAbort.setText(ScreenFileTransferView.this
						.getString(R.string.break_off));
			...
			case SUCCESS_REPORT:
			case DATA:
				if ((mSession.isOutgoing() && type == NgnMsrpEventTypes.SUCCESS_2XX)
						|| (!mSession.isOutgoing() && type == NgnMsrpEventTypes.DATA)) {
					updateProgressBar(
							intent.getLongExtra(
									NgnMsrpEventArgs.EXTRA_BYTE_RANGE_START,
									-1L),
							intent.getLongExtra(
									NgnMsrpEventArgs.EXTRA_BYTE_RANGE_END, -1L),
							intent.getLongExtra(
									NgnMsrpEventArgs.EXTRA_BYTE_RANGE_TOTAL,
									-1L));
				}
				break;
			case ERROR:
	...
	public static boolean sendFile(String remoteUri, String filePath) { // /mnt/sdcard/DCIM/Camera/IMG_20140610_151900.jpg
		...
		final NgnMsrpSession msrpSession = NgnMsrpSession
				.createOutgoingSession(sipService.getSipStack(),
						NgnMediaType.FileTransfer, validUri);
		if (msrpSession == null) {
			Log.e(TAG, "Failed to create MSRP session");
			return false;
		}
		msrpSession.addHeader("APP_TYPE", "IMS messaging/Large Message-mode");
		if (msrpSession.sendFile(filePath)) { // /storage/sdcard0/DCIM/Camera/IMG_20120508_221732.jpg
			Log.e(TAG, "-----sendFile session-----");
			Log.e(TAG, "" + msrpSession.getId());
			Log.e(TAG, "" + msrpSession.getRemotePartyUri());
			screenService.show(ScreenFileTransferView.class,
					Long.toString(msrpSession.getId()));
			return true;
		} else {
